	...
	public function execute()
	{
		...
		{  
			...
			try {
				...
				$client_id = $this->config->getValue("payment/jPay_gateway/client_id",$storeScope);
				$client_secret = $this->config->getValue("payment/jPay_gateway/client_secret",$storeScope);
				$testmode = $this->config->getValue("payment/jPay_gateway/instamojo_testmode",$storeScope);
				$this->logger->info("Client ID: $client_id | Client Secret : $client_secret | Testmode: $testmode");
				
				# use instamojo library
				$ds = DIRECTORY_SEPARATOR;
				include __DIR__ . "$ds..$ds..$ds/lib/Jumia.php";
						...
						if($payment_status == "successful")
						{
							...
							if($transaction)
							{
								...
								$transaction->setAdditionalInformation(  
									"jPay Transaction Id",$payment_id
								);
								$transaction->setAdditionalInformation(  
									"status","successful"
								);
				...
				{
					$this->inbox->addCritical("jPay Authorization Error","Please contact to jPAy for troubleshooting. ".$e->getMessage());
				}
